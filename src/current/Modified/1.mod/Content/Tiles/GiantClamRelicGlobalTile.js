...
export class GiantClamRelicGlobalTile extends GlobalTile {
    GetSystem() {
        return ModSystem.getByName('GiantClamRelicSystem');
    }
    ...
    OnPlace(player, i, j, type, style) {
        ...
        if (!system)
            return;
        const customType = Number(ModItem.getTypeByName('GiantClamRelic') || 0);
    ...
    DropCustomRelic(system, position, noItem) {
        ...
        if (customType > 0) {
            try {
                ...
                tl.log(`[CalamityPort] Slime God relic custom drop failed: ${e}`);
            }
        }
    }
