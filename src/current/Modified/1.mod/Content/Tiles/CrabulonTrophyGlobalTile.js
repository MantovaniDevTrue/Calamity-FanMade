...
export class CrabulonTrophyGlobalTile extends GlobalTile {
    GetSystem() {
        return ModSystem.getByName('CrabulonTrophySystem');
    }
    ...
    OnPlace(player, i, j, type, style) {
        ...
        if (!system)
            return;
        const customType = Number(ModItem.getTypeByName('CrabulonTrophy') || 0);
    ...
    CanDropItems(i, j, tile) {
        ...
        if (customType > 0) {
            try {
                ...
                tl.log(`[CalamityPort] Crabulon trophy custom drop failed: ${e}`);
            }
        }
        return false;
    }
