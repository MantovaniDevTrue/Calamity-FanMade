...
export class HiveMindTrophyGlobalTile extends GlobalTile {
    GetSystem() {
        return ModSystem.getByName('HiveMindTrophySystem');
    }
    ...
    OnPlace(player, i, j, type, style) {
        ...
        if (!system)
            return;
        const customType = Number(ModItem.getTypeByName('HiveMindTrophy') || 0);
    ...
    CanDropItems(i, j, tile) {
        ...
        if (customType > 0) {
            try {
                ...
                tl.log(`[CalamityPort] HiveMind trophy custom drop failed: ${e}`);
            }
        }
        return false;
    }
