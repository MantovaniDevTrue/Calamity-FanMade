...
export class GiantClamTrophyGlobalTile extends GlobalTile {
    GetSystem() {
        return ModSystem.getByName('GiantClamTrophySystem');
    }
    ...
    OnPlace(player, i, j, type, style) {
        ...
        if (!system)
            return;
        const customType = Number(ModItem.getTypeByName('GiantClamTrophy') || 0);
    ...
    CanDropItems(i, j, tile) {
        ...
        if (customType > 0) {
            try {
                ...
                tl.log(`[CalamityPort] Slime God trophy custom drop failed: ${e}`);
            }
        }
        return false;
    }
