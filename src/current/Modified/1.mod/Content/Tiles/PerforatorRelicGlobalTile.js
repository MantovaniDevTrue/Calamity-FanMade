...
export class PerforatorRelicGlobalTile extends GlobalTile {
    GetSystem() {
        return ModSystem.getByName('PerforatorRelicSystem');
    }
    ...
    OnPlace(player, i, j, type, style) {
        ...
        if (!system)
            return;
        const customType = Number(ModItem.getTypeByName('PerforatorsRelic') || 0);
    ...
    DropCustomRelic(system, position, noItem) {
        ...
        if (customType > 0) {
            try {
                ...
                tl.log(`[CalamityPort] Perforators relic custom drop failed: ${e}`);
            }
        }
    }
