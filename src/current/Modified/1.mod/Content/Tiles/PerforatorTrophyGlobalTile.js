...
export class PerforatorTrophyGlobalTile extends GlobalTile {
    GetSystem() {
        return ModSystem.getByName('PerforatorTrophySystem');
    }
    ...
    OnPlace(player, i, j, type, style) {
        ...
        if (!system)
            return;
        const customType = Number(ModItem.getTypeByName('PerforatorTrophy') || 0);
    ...
    CanDropItems(i, j, tile) {
        ...
        if (customType > 0) {
            try {
                ...
                tl.log(`[CalamityPort] Perforator trophy custom drop failed: ${e}`);
            }
        }
        return false;
    }
