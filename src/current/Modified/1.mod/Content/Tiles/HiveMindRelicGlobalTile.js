...
export class HiveMindRelicGlobalTile extends GlobalTile {
    GetSystem() {
        return ModSystem.getByName('HiveMindRelicSystem');
    }
    ...
    OnPlace(player, i, j, type, style) {
        ...
        if (!system)
            return;
        const customType = Number(ModItem.getTypeByName('HiveMindRelic') || 0);
    ...
    DropCustomRelic(system, position, noItem) {
        ...
        if (customType > 0) {
            try {
                ...
                tl.log(`[CalamityPort] Hive Mind relic custom drop failed: ${e}`);
            }
        }
    }
