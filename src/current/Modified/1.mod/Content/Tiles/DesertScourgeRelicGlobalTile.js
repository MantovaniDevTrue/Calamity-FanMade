...
export class DesertScourgeRelicGlobalTile extends GlobalTile {
    GetSystem() {
        return ModSystem.getByName('DesertScourgeRelicSystem');
    }
    ...
    OnPlace(player, i, j, type, style) {
        ...
        if (!system)
            return;
        const customType = Number(ModItem.getTypeByName('DesertScourgeRelic') || 0);
    ...
    DropCustomRelic(system, position, noItem) {
        ...
        if (customType > 0) {
            try {
                ...
                tl.log(`[CalamityPort] Desert Scourge relic custom drop failed: ${e}`);
            }
        }
    }
