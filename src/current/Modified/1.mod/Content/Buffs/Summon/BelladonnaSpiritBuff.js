...
export class BelladonnaSpiritBuff extends ModBuff {
    constructor(){super();this.Texture='Buffs/Summon/BelladonnaSpiritBuff';this.MinionType=0;}
    ...
    PostSetupContent(){this.MinionType=Number(ModProjectile.getTypeByName('BelladonnaSpirit')||0);}
    UpdatePlayer(player,buffIndex){
        if(!player||player.dead){try{player?.DelBuff(buffIndex);}catch(_){}return;}
        if(!(this.MinionType>0))this.MinionType=Number(ModProjectile.getTypeByName('BelladonnaSpirit')||0);
    ...
    OnRemove(player){if(!(this.MinionType>0))this.MinionType=Number(ModProjectile.getTypeByName('BelladonnaSpirit')||0);killOwned(player,this.MinionType);}
}
