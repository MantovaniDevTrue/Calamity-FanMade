...
export class CinderBlossomBuff extends ModBuff{
 constructor(){super();this.Texture='Buffs/Summon/CinderBlossomBuff';this.MinionType=0;}
 ...
 PostSetupContent(){this.MinionType=Number(ModProjectile.getTypeByName('CinderBlossom')||0);}
 UpdatePlayer(player,buffIndex){if(!player||player.dead){try{if(player)player.DelBuff(buffIndex);}catch(_){}return;}if(!(this.MinionType>0))this.MinionType=Number(ModProjectile.getTypeByName('CinderBlossom')||0);if(this.MinionType>0&&CountOwned(player,this.MinionType)>0){try{player.buffTime[buffIndex]=18000;}catch(_){try{player.buffTime.set_Item(buffIndex,18000);}catch(__){}}}else try{player.DelBuff(buffIndex);}catch(_){} }
 OnRemove(player){if(!(this.MinionType>0))this.MinionType=Number(ModProjectile.getTypeByName('CinderBlossom')||0);killOwned(player,this.MinionType);}
}
