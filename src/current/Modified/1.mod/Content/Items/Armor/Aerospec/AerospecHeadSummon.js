...
export class AerospecHeadSummon extends ModItem {
    constructor() { super(); this.Texture = 'Items/Armor/Aerospec/AerospecHeadSummon'; this.ResearchUnlockCount = 1; }
    SetDefaults() {
        const item = this.Item; item.width = 18; item.height = 18; item.maxStack = 1;
        item.value = Terraria.Item.buyPrice(0, 5, 0, 0); item.rare = Terraria.ID.ItemRarityID.Orange; item.defense = 2;
    }
    ...
    UpdateArmorSet(item, player) {
        player.setBonus = ModLocalization.getTranslationArmorSetBonus('AerospecSummon');
        ApplyAerospecSet(player, 'summon');
    }
