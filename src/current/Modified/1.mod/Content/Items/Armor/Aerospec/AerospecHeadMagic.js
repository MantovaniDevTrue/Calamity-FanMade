...
export class AerospecHeadMagic extends ModItem {
    constructor() { super(); this.Texture = 'Items/Armor/Aerospec/AerospecHeadMagic'; this.ResearchUnlockCount = 1; }
    SetDefaults() {
        const item = this.Item; item.width = 18; item.height = 18; item.maxStack = 1;
        item.value = Terraria.Item.buyPrice(0, 5, 0, 0); item.rare = Terraria.ID.ItemRarityID.Orange; item.defense = 3;
    }
    ...
    UpdateArmorSet(item, player) {
        player.setBonus = ModLocalization.getTranslationArmorSetBonus('AerospecMagic');
        ApplyAerospecSet(player, 'magic');
    }
