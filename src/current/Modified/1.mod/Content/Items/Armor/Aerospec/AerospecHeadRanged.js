...
export class AerospecHeadRanged extends ModItem {
    constructor() { super(); this.Texture = 'Items/Armor/Aerospec/AerospecHeadRanged'; this.ResearchUnlockCount = 1; }
    SetDefaults() {
        const item = this.Item; item.width = 18; item.height = 18; item.maxStack = 1;
        item.value = Terraria.Item.buyPrice(0, 5, 0, 0); item.rare = Terraria.ID.ItemRarityID.Orange; item.defense = 5;
    }
    ...
    UpdateArmorSet(item, player) {
        player.setBonus = ModLocalization.getTranslationArmorSetBonus('AerospecRanged');
        ApplyAerospecSet(player, 'ranged');
    }
