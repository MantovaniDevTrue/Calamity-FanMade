...
export class CorroslimeStaff extends ModItem {
    constructor() {
        super();
        this.Texture = 'Items/Weapons/Summon/CorroslimeStaff';
    ...
    SetDefaults() {
        ...
        i.autoReuse = true;
        i.buffType = ModBuff.getTypeByName('Corroslime');
        i.shoot = ModProjectile.getTypeByName('CorroslimeMinion');
        i.shootSpeed = 0;
        this.MenuCategories.push('summon');
    }

    Shoot(item, player, position, velocity, type, damage, knockBack) {
        const b = Number(ModBuff.getTypeByName('Corroslime') || 0), p = Number(ModProjectile.getTypeByName('CorroslimeMinion') || 0);
        if (!(b > 0 && p > 0))
            return false;
