...
export class SaharaSlicersUIHooks extends GlobalHooks {
    constructor() {
        super();
        this.Pixel = null;
    ...
    LoadTextures() {
        try {
            ...
            this.BoltTexture = tl.texture.load('Textures/Projectiles/Melee/SaharaSlicersBolt.png');
            this.TexturesLoaded = !!this.Pixel;
        } catch (e) {
            this.TexturesLoaded = false;
            tl.log(`[CalamityPort] Sahara Slicers button texture load failed: ${e}`);
        }
    }
    ...
    Fire(state, player, source, x, y) {
        const tick = Number(Main.GameUpdateCount || 0);
        if (tick - this.LastFireTick < 8)
            return false;
        ...
        return state.FireSaharaSlicersBolt(player, false);
    }
    ...
    ProcessInput(state, player, button) {
        ...
        if (rawTrigger) {
            ...
            this.Fire(state, player, 'raw-touch', rawX, rawY);
            return;
        }
        if (worldInside && worldReleased) {
            ...
            this.Fire(state, player, 'world-touch', worldX, worldY);
            return;
        }
        if (mouseInside) {
            this.UnifiedTouchLocked = true;
            this.Fire(state, player, 'mouse-fallback', Main.mouseX, Main.mouseY);
        }
    }
    ...
    DrawButton(state, count, button) {
        ...
        const border = ready ? Color.Gold : Color.Gray;
        const inner = ready ? Tint(Color.Gold, 0.28) : Tint(Color.Gray, 0.18);
        ...
        if (this.BoltTexture)
            DrawTexture(this.BoltTexture, left + 23 * button.scale, button.y, 0.68 * button.scale, Color.White);
        const textX = button.x + 12 * button.scale;
        DrawCenteredText(`DARDOS: ${count}/10`, textX, button.y - 7 * button.scale, 0.36 * button.scale, Color.White);
        DrawCenteredText(ready ? 'LANÇA NA DIREÇÃO DO ÚLTIMO ACERTO' : 'ACERTE COM AS ADAGAS', textX, button.y + 8 * button.scale, 0.23 * button.scale, ready ? Color.LightGoldenrodYellow : Color.LightGray);
    }
    ...
    Draw() {
        ...
        if (!(this.CachedItemType > 0))
            this.CachedItemType = Number(ModItem.getTypeByName('SaharaSlicers') || 0);
        const itemType = this.CachedItemType;
        if (!player || !player.active || player.dead || !state || !(itemType > 0))
            return;
        let heldType = 0;
        try {
            heldType = Number(player.HeldItem.type);
        } catch (e) { }
        if (heldType !== Number(itemType))
            return;
        const count = Math.max(0, Math.min(10, Math.floor(Number(state.SaharaSlicersBolts) || 0)));
