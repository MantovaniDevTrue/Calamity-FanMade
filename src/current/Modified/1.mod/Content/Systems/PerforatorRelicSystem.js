...
export class PerforatorRelicSystem extends ModSystem {
    constructor() {
        super();
        this.Positions = [];
    ...
    static StorageKey() {
        return 'calamity:world:perforatorRelics';
    }
    ...
    OnWorldLoad() {
        ...
        this.LastCleanupTick = -1000;
        this.LastClaimStatus = 'idle';
        const raw = WorldDB.get(PerforatorRelicSystem.StorageKey());
        ...
        try {
            ...
            tl.log(`[CalamityPort] Perforator relic position load failed: ${e}`);
        }
    }
    ...
    SavePositions() {
        try {
            WorldDB.set(PerforatorRelicSystem.StorageKey(), JSON.stringify(this.Positions.map(p => ({ x: Math.floor(p.x), y: Math.floor(p.y) }))));
        } catch (e) {
            tl.log(`[CalamityPort] Perforator relic position save failed: ${e}`);
        }
    }
