...
export class HiveMindRelicSystem extends ModSystem {
    constructor() {
        super();
        this.Positions = [];
    ...
    static StorageKey() {
        return 'calamity:world:hiveMindRelics';
    }
    ...
    OnWorldLoad() {
        ...
        this.LastCleanupTick = -1000;
        this.LastClaimStatus = 'idle';
        const raw = WorldDB.get(HiveMindRelicSystem.StorageKey());
        ...
        try {
            ...
            tl.log(`[CalamityPort] HiveMind relic position load failed: ${e}`);
        }
    }
    ...
    SavePositions() {
        try {
            WorldDB.set(HiveMindRelicSystem.StorageKey(), JSON.stringify(this.Positions.map(p => ({ x: Math.floor(p.x), y: Math.floor(p.y) }))));
        } catch (e) {
            tl.log(`[CalamityPort] HiveMind relic position save failed: ${e}`);
        }
    }
    ...
    GetCustomItemType() {
        return Number(ModItem.getTypeByName('HiveMindRelic') || 0);
    }
