...
export class CrabulonRelicSystem extends ModSystem {
    constructor() {
        super();
        this.Positions = [];
    ...
    static StorageKey() {
        return 'calamity:world:crabulonRelics';
    }
    ...
    OnWorldLoad() {
        ...
        this.LastCleanupTick = -1000;
        this.LastClaimStatus = 'idle';
        const raw = WorldDB.get(CrabulonRelicSystem.StorageKey());
        ...
        try {
            ...
            tl.log(`[CalamityPort] Crabulon relic position load failed: ${e}`);
        }
    }
    ...
    SavePositions() {
        try {
            WorldDB.set(CrabulonRelicSystem.StorageKey(), JSON.stringify(this.Positions.map(p => ({ x: Math.floor(p.x), y: Math.floor(p.y) }))));
        } catch (e) {
            tl.log(`[CalamityPort] Crabulon relic position save failed: ${e}`);
        }
    }
    ...
    GetCustomItemType() {
        return Number(ModItem.getTypeByName('CrabulonRelic') || 0);
    }
