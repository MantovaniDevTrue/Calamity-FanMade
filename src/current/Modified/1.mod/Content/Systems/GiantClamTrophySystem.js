...
export class GiantClamTrophySystem extends ModSystem {
    constructor() {
        super();
        this.Positions = [];
    ...
    static StorageKey() {
        return 'calamity:world:giantClamTrophies';
    }
    ...
    OnWorldLoad() {
        ...
        this.PendingTick = -1000;
        this.LastCleanupTick = -1000;
        const raw = WorldDB.get(GiantClamTrophySystem.StorageKey());
    ...
    SavePositions() {
        try {
            WorldDB.set(GiantClamTrophySystem.StorageKey(), JSON.stringify(this.Positions.map(p => ({ x: Math.floor(p.x), y: Math.floor(p.y) }))));
        } catch (e) {
    ...
    GetCustomItemType() {
        return Number(ModItem.getTypeByName('GiantClamTrophy') || 0);
    }
}
