...
export class PerforatorTrophySystem extends ModSystem {
    constructor() {
        super();
        this.Positions = [];
    ...
    static StorageKey() {
        return 'calamity:world:perforatorTrophies';
    }
    ...
    OnWorldLoad() {
        ...
        this.PendingTick = -1000;
        this.LastCleanupTick = -1000;
        const raw = WorldDB.get(PerforatorTrophySystem.StorageKey());
        ...
        try {
            ...
            tl.log(`[CalamityPort] Perforator trophy position load failed: ${e}`);
        }
    }
    ...
    SavePositions() {
        try {
            WorldDB.set(PerforatorTrophySystem.StorageKey(), JSON.stringify(this.Positions.map(p => ({ x: Math.floor(p.x), y: Math.floor(p.y) }))));
        } catch (e) {
            tl.log(`[CalamityPort] Perforator trophy position save failed: ${e}`);
        }
    }
