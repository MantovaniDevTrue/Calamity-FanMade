...
import { FillSurfaceShrineChestByIndex } from './../../Core/SurfaceShrineRuntime.js';
...
function IsInside(x, y) {
    ...
    const width = Math.max(1, NumberOr(WorldDB.get(KEY + 'width'), 56)), height = Math.max(1, NumberOr(WorldDB.get(KEY + 'height'), 36));
    return left >= 0 && top >= 0 && x >= left && x < left + width && y >= top && y < top + height;
}
function Log(message) { try { tl.log(`[CalamityPort SurfaceShrineRepair] ${message}`); } catch (e) { } }

export class SurfaceShrineChestRepairSystem extends ModSystem {
    ...
    Update() {
        ...
        const filled = FillSurfaceShrineChestByIndex(chestIndex);
