...
export class SlimeGodRelicSystem extends ModSystem {
    constructor() {
        super();
        this.Positions = [];
    ...
    static StorageKey() {
        return 'calamity:world:slimeGodRelics';
    }
    ...
    OnWorldLoad() {
        ...
        this.LastCleanupTick = -1000;
        this.LastClaimStatus = 'idle';
        const raw = WorldDB.get(SlimeGodRelicSystem.StorageKey());
    ...
    SavePositions() {
        try {
            WorldDB.set(SlimeGodRelicSystem.StorageKey(), JSON.stringify(this.Positions.map(p => ({ x: Math.floor(p.x), y: Math.floor(p.y) }))));
        } catch (e) {
