...
export class HiveMindTrophySystem extends ModSystem {
    constructor() {
        super();
        this.Positions = [];
    ...
    static StorageKey() {
        return 'calamity:world:hiveMindTrophies';
    }
    ...
    OnWorldLoad() {
        ...
        this.PendingTick = -1000;
        this.LastCleanupTick = -1000;
        const raw = WorldDB.get(HiveMindTrophySystem.StorageKey());
        ...
        try {
            ...
            tl.log(`[CalamityPort] HiveMind trophy position load failed: ${e}`);
        }
    }
    ...
    SavePositions() {
        try {
            WorldDB.set(HiveMindTrophySystem.StorageKey(), JSON.stringify(this.Positions.map(p => ({ x: Math.floor(p.x), y: Math.floor(p.y) }))));
        } catch (e) {
            tl.log(`[CalamityPort] HiveMind trophy position save failed: ${e}`);
        }
    }
    ...
    GetCustomItemType() {
        return Number(ModItem.getTypeByName('HiveMindTrophy') || 0);
    }
}
