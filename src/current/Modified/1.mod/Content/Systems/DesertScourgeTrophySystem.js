...
export class DesertScourgeTrophySystem extends ModSystem {
    constructor() {
        super();
        this.Positions = [];
    ...
    static StorageKey() {
        return 'calamity:world:desertScourgeTrophies';
    }
    ...
    OnWorldLoad() {
        ...
        this.PendingTick = -1000;
        this.LastCleanupTick = -1000;
        const raw = WorldDB.get(DesertScourgeTrophySystem.StorageKey());
        ...
        try {
            ...
            tl.log(`[CalamityPort] Desert Scourge trophy position load failed: ${e}`);
        }
    }
    ...
    SavePositions() {
        try {
            WorldDB.set(DesertScourgeTrophySystem.StorageKey(), JSON.stringify(this.Positions.map(p => ({ x: Math.floor(p.x), y: Math.floor(p.y) }))));
        } catch (e) {
            tl.log(`[CalamityPort] Desert Scourge trophy position save failed: ${e}`);
        }
    }
    ...
    GetCustomItemType() {
        return Number(ModItem.getTypeByName('DesertScourgeTrophy') || 0);
    }
}
