...
export class CrabulonTrophySystem extends ModSystem {
    constructor() {
        super();
        this.Positions = [];
    ...
    static StorageKey() {
        return 'calamity:world:crabulonTrophies';
    }
    ...
    OnWorldLoad() {
        ...
        this.PendingTick = -1000;
        this.LastCleanupTick = -1000;
        const raw = WorldDB.get(CrabulonTrophySystem.StorageKey());
        ...
        try {
            ...
            tl.log(`[CalamityPort] Crabulon trophy position load failed: ${e}`);
        }
    }
    ...
    SavePositions() {
        try {
            WorldDB.set(CrabulonTrophySystem.StorageKey(), JSON.stringify(this.Positions.map(p => ({ x: Math.floor(p.x), y: Math.floor(p.y) }))));
        } catch (e) {
            tl.log(`[CalamityPort] Crabulon trophy position save failed: ${e}`);
        }
    }
    ...
    GetCustomItemType() {
        return Number(ModItem.getTypeByName('CrabulonTrophy') || 0);
    }
}
