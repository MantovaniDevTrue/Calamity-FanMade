...
export class DesertScourgeRelicSystem extends ModSystem {
    constructor() {
        super();
        this.Positions = [];
    ...
    static StorageKey() {
        return 'calamity:world:desertScourgeRelics';
    }
    ...
    OnWorldLoad() {
        ...
        this.LastCleanupTick = -1000;
        this.LastClaimStatus = 'idle';
        const raw = WorldDB.get(DesertScourgeRelicSystem.StorageKey());
        ...
        try {
            ...
            tl.log(`[CalamityPort] Desert Scourge relic position load failed: ${e}`);
        }
    }
    ...
    SavePositions() {
        try {
            WorldDB.set(DesertScourgeRelicSystem.StorageKey(), JSON.stringify(this.Positions.map(p => ({ x: Math.floor(p.x), y: Math.floor(p.y) }))));
        } catch (e) {
            tl.log(`[CalamityPort] Desert Scourge relic position save failed: ${e}`);
        }
    }
    ...
    GetCustomItemType() {
        return Number(ModItem.getTypeByName('DesertScourgeRelic') || 0);
    }
