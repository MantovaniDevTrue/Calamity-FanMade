...
function IsInside(x, y) {
    ...
    const width = Math.max(1, NumberOr(WorldDB.get(KEY + 'width'), 23)), height = Math.max(1, NumberOr(WorldDB.get(KEY + 'height'), 44));
    return left >= 0 && top >= 0 && x >= left && x < left + width && y >= top && y < top + height;
}
function Log(message) { try { tl.log(`[CalamityPort MarbleShrineRepair] ${message}`); } catch (e) { } }

export class MarbleShrineChestRepairSystem extends ModSystem {
    ...
    Update() {
        ...
        const filled = drunk ? FillGraniteShrineChestByIndex(chestIndex) : FillMarbleShrineChestByIndex(chestIndex);
