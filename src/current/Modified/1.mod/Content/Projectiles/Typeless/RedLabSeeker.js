...
const KEY='calamity:structure:underworldLab:';
...
export function RedLabSeekerActive(owner){const o=I(owner,-1),now=Tick(),s=ActiveOwners.get(o);if(!s)return false;if(s.until<now){ActiveOwners.delete(o);return false;}return true;}
export class RedLabSeeker extends ModProjectile{
 constructor(){super();this.Texture='Items/LabFinders/RedSeekingMechanism';}
 ...
 AI(p){if(!Refresh(p)){p.timeLeft=1;try{p.Kill();}catch(e){}return;}const d=Destination();if(!d){p.timeLeft=1;return;}const ai=new ProjAI(p),time=N(ai[0],0),c=Center(p);let vx=N(p.velocity?.X),vy=N(p.velocity?.Y),cx=c.x,cy=c.y;
  ...
  try{const cc=Center(p);Terraria.Lighting.AddLight(Math.floor(cc.x/16),Math.floor(cc.y/16),0.95,0.15,0.10);}catch(e){}
  ai[0]=time+1;
 }
