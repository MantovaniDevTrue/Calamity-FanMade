...
export function CyanLabSeekerActive(owner){const o=I(owner,-1),now=Tick(),s=ActiveOwners.get(o);if(!s)return false;if(s.until<now){ActiveOwners.delete(o);return false;}return true;}
export class CyanLabSeeker extends ModProjectile{
 constructor(){super();this.Texture='Projectiles/Typeless/CyanLabSeeker';}
 ...
 AI(p){if(!Refresh(p)){p.timeLeft=1;try{p.Kill();}catch(e){}return;}const d=Destination();if(!d){p.timeLeft=1;return;}const ai=new ProjAI(p),time=N(ai[0],0),c=Center(p);let vx=N(p.velocity?.X),vy=N(p.velocity?.Y),cx=c.x,cy=c.y;
  ...
  try{const cc=Center(p);Terraria.Lighting.AddLight(Math.floor(cc.x/16),Math.floor(cc.y/16),0.2,0.85,0.95);}catch(e){}
  ai[0]=time+1;
 }
