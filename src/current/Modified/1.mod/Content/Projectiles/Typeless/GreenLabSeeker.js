...
export function GreenLabSeekerActive(owner){const o=I(owner,-1),now=Tick(),s=ActiveOwners.get(o);if(!s)return false;if(s.until<now){ActiveOwners.delete(o);return false;}return true;}
export class GreenLabSeeker extends ModProjectile{
 constructor(){super();this.Texture='Projectiles/Typeless/GreenLabSeeker';}
 ...
 AI(p){if(!Refresh(p)){p.timeLeft=1;try{p.Kill();}catch(e){}return;}const d=Destination();if(!d){p.timeLeft=1;return;}const ai=new ProjAI(p),time=N(ai[0],0),c=Center(p);let vx=N(p.velocity?.X),vy=N(p.velocity?.Y),cx=c.x,cy=c.y;
  ...
  else if(time>=80){ai[1]=0;const dir=N(p.rotation)-Math.PI/2,speed=Math.max(0,Math.min(1,(time-80)/25))*25;p.velocity=Vector2.new(Math.cos(dir)*speed,Math.sin(dir)*speed);if(time%3===0){try{const cc=Center(p);const du=Terraria.Dust.QuickDust(Vector2.new(cc.x+(Math.random()*16-8),cc.y+(Math.random()*16-8)),Modules.Color.Green);if(du){du.noGravity=true;du.scale=1.25;}}catch(e){}}}
  try{const cc=Center(p);Terraria.Lighting.AddLight(Math.floor(cc.x/16),Math.floor(cc.y/16),0.12,0.8,0.12);}catch(e){}
  ai[0]=time+1;
 }
 CanDamage(){return false;}
 CanCutTiles(){return false;}
 OnKill(p){Release(p);for(let i=0;i<4;i++){try{const c=Center(p);Terraria.Dust.QuickDust(Vector2.new(c.x+(Math.random()*24-12),c.y+(Math.random()*24-12)),Modules.Color.Green);}catch(e){}}}
}
